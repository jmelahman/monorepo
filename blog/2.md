---
og-title: "Using `uv` to package Go applications"
title-prefix: "Blog"
date: 2025-08-12
nav_blog: true
---
...
Also, this approach is mutually-exclusive to the default Go packaging which remains available to
users if preferable.

The majority of my complaints are really just, "Go doesn't have a canonical package repository":

1. Installing Go packages requires Go be installed
2. Installing Go packages requires a specific version of Go be installed
3. Installing Go packages requires building them from source
...
For most, basic Go applications, deployments are a simple [Github Actions workflow](https://github.com/goreleaser/goreleaser-action?tab=readme-ov-file#usage),
...
There are only 2, minor issues with GoReleaser,

1. There isn't a ubiquitous, CLI solution for downloading these artifacts from Github.
a. This implies the platform detection is not automatic and therefore error-prone.
...
### `manygo` and the power of small, python libraries
...
Initially, the `hatch_build.py` above invoked `go` from the command-line.
