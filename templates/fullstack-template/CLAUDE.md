...
- Playwright E2E: `cd web && bun run test:e2e:install && bun run test:e2e`
  (the install step fetches Chromium into `~/.cache` once; the suite boots
  the real backend with `--in-memory` plus the Vite dev server).
